...
namespace Test.Controllers
{
    ...
    public class TeacherController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? search)
        {
            ...
            TempData["search for specific Teacher"] = search?.ToUpper(); // Store the uppercased TeacherName in a local variable
            ViewData["checking"] = false;


            if (!string.IsNullOrEmpty(search))
            {
                TeacherList = (IList<ApplicationUser>)TeacherList.Where(u => u.UserName.StartsWith(search));
                if (TeacherList == null)
                {
                    ViewData["checking"] = true;
                    TeacherList = await userManager.GetUsersInRoleAsync("Teacher");
                }
            }
            ...
            var paginatedList = TeacherList.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToList();
            ViewData["PageNumber"] = pageNumber;
            ViewData["TotalPages"] = (int)Math.Ceiling(TeacherList.ToList().Count() / (double)pageSize);
        ...
        public async Task<IActionResult> Create(RegisterUserViewModel UserVM, IFormFile image)
        {
            if (ModelState.IsValid && image != null && image.Length > 0)
            {
                ...
                string imagePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Images/Teacher", fileName);
                ...
                {
                    IdentityResult result2 = await userManager.AddToRoleAsync(user, "Teacher");
                    //not create cookie
        ...
        {
            // Retrieve the Teacher entity from your data store based on the provided ID
        ...
        public async Task<IActionResult> Edit(RegisterUserViewModel UserVM, string id, IFormFile image)
        {
            if (ModelState.IsValid && UserVM != null)
            {
                ...
                if (image != null && image.Length > 0)
                {
                    ...
                    {
                        string oldImagePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Images/Teacher", existinguser.ImageUrl);
        ...
        public async Task<IActionResult> DeleteConfirmed(string id)
        {
            ...
            {
                string imagePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Images/Teacher", user.ImageUrl);
            ...
            IdentityResult result = await userManager.RemoveFromRoleAsync(user, "Teacher");
            if (result.Succeeded)
            {
